...
なお、今回紹介した Recoil + dataloader は、 Render-as-You-Fetch パターンを実現する理想のライブラリ **ではありません。** 

ではなぜこの例を出したかというと、単純に **執筆時間の問題でもっと便利なライブラリを使っての解説を書けるほど調査に時間を割けなかった** というのが大きいですね・・・。まあ業務で実際に使っているアーキテクチャなので実績はあります、一応。

僕のおすすめはやはり [TanStack Query](https://tanstack.com/query/latest) や [useSWR](https://swr.vercel.app/ja) がシンプルかつ高機能で良いのではないかと思います。


### __それではJSON 色付け戦士のみなさまの明日に幸あれ・・・__
